    ...
    if request.method == "POST":
        ...
        if data['stu_first_name']:
            students = students.filter(
                first_name__contains=data["stu_first_name"])
        if data['stu_last_name']:
            students = students.filter(
                last_name__contains=data["stu_last_name"])
...
class SessionConstants:
    opt_hours = '\n'.join(
        ["<option value='{0:02d}'>{0:02d}</option>".format(i) for i in range(7, 21)])
    opt_minutes = '\n'.join(
        ["<option value='{0:02d}'>{0:02d}</option>".format(i) for i in range(0, 60, 15)])
...
def edit_session(request):
    if request.method == "POST":
        ...
        hour_options = SessionConstants.opt_hours.replace(
            "value='%s'" % sh, "value='%s' selected='selected'" % sh)  # Set default as the selected value
        minute_options = SessionConstants.opt_minutes.replace(
            "value='%s'" % sm, "value='%s' selected='selected'" % sm)
        hour_options_1 = SessionConstants.opt_hours.replace(
            "value='%s'" % eh, "value='%s' selected='selected'" % eh)
        minute_options_1 = SessionConstants.opt_minutes.replace(
            "value='%s'" % em, "value='%s' selected='selected'" % em)
        context.update(
            {
        ...
        if student_query.isdigit():
            matched_student = StudentAccount.objects.filter(
                student_id__exact=student_query)
            if len(matched_student) == 0:
                ...
                context['student_info'] = matched_student[0].last_name.upper(
                ) + ', ' + matched_student[0].first_name
        else:
            context['form_valid'] = False
        ...
        if advisor_query.isdigit():
            matched_advisor = StaffAccount.objects.filter(
                staff_id__exact=advisor_query)
            if len(matched_advisor) == 0:
                ...
                context['advisor_info'] = matched_advisor[0].last_name.upper(
                ) + ', ' + matched_advisor[0].first_name
        else:
            context['form_valid'] = False
        ...
        if context['form_valid'] and data['confirm_booking'] == 'yes':
            date_ = date(y, m, d)
            start_time = datetime(y, m, d, int(
                sh), int(sm), tzinfo=timezone.utc)
...
def create_session(request):
    if request.method == "POST":
        ...
        if data['confirm_booking'] == 'yes':
            ...
            html_message = render_to_string('email/email.html', {'date': date_.strftime("%d/%m/%y"), 'starttime': start_time.strftime(
                '%I:%M %p'), 'endtime': end_time.strftime('%I:%M %p'), 'location': context['default_location'], 'staffname': matched_advisor.first_name, 'word': 'booked'})
            plain_message = strip_tags(html_message)
            emailContent = {
                'subject': 'Confirming your UTS HELPS session with {} on {} {}'.format(matched_advisor.first_name, date_.strftime("%d/%m/%y"), start_time.strftime('%I:%M %p')),
                'html_message': html_message,
                'plain_message': plain_message,
                'contacts': [matched_student.email, matched_advisor.email]
            }
            return render(request, 'pages/layouts/session_booked.html', context)
        else:
            context['page_title'] = 'Confirm Booking' if context['form_valid'] else 'Book a Session'
            context['book_or_edit'] = 'Book'
            context['calendar'] = mark_safe(SessionConstants.calendar.new_date(
                y, m, d).formatmonth(True, context['prev_month'], context['next_month']))
        ...
        html_cal = cal.formatmonth(
            True, context['prev_month'], context['next_month'])
...
def create_workshop(request):
    if request.method == "POST":
        ...
        # print (context)
        html_cal = cal.formatmonth(
            True, context['prev_month'], context['next_month'], False)
